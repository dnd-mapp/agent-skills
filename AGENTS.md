...
Run the `address-comments` skill ([skills/address-comments/SKILL.md](skills/address-comments/SKILL.md)) to work through unresolved review threads and unaddressed general PR comments with code changes, replies, and thread resolutions. It has no configuration file. The PR author runs it on their own PR, so replies and thread resolutions post as the active `gh` account rather than a configured bot.
