...
if [[ ! "$subject_line" =~ $subject_re ]]; then
    echo "VIOLATION: Subject line does not match expected format 'type(scope)!: Subject' — no recognisable type prefix found" >&2
    exit 1
fi
...
# ---------------------------------------------------------------------------
if [[ -n "$scope_raw" ]]; then
    scope_inner="${scope_raw:1:${#scope_raw}-2}"
...
# ---------------------------------------------------------------------------
# 4. Subject text — capital first letter
# ---------------------------------------------------------------------------
first_char="${subject_text:0:1}"

if [[ "$first_char" =~ [a-z] ]]; then
    violations+=("Subject must start with a capital letter (got '${first_char}')")
fi

# ---------------------------------------------------------------------------
# 5. Subject text — no trailing period
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Subject line total length ≤ 50
# ---------------------------------------------------------------------------
subject_len=${#subject_line}

if [[ $subject_len -gt $MAX_SUBJECT ]]; then
    violations+=("Subject line is ${subject_len} characters — max is ${MAX_SUBJECT}. Rewrite to be more concise while preserving intent.")
fi
