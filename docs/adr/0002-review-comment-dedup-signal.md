...
- **Diffing the code at the flagged location against its state when the old comment was posted.** Rejected: requires tracking the original commit per comment, and inferring sameness from code shape. That's strictly more machinery than reading two signals GitHub already tracks per thread. This rejection is about per-comment history tracking. It does not cover reading the *current* diff to judge whether a point made only in a review summary or PR comment has since been answered. That point has no thread and no resolved bit, so the live diff and any later author reply are the only signals. That is the same reading-comprehension judgment used elsewhere, with no history to track.
